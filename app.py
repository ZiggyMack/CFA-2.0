...
def show_landing_page():
    ...
    st.markdown('<p class="quote">"All Named. All Priced."</p>', unsafe_allow_html=True)
    
    st.markdown("""
        <div class="manifesto">
            <div class="manifesto-title">⚡️ Launch Manifesto</div>
            ...
            <p><strong>🧩 What You're Touching</strong></p>
            <p>The CFA-Voodoo Console is not a game of opinions. It's an instrument—a system that:</p>
            <ul>
                <li>Quantifies coherence, depth, fertility, beauty, and morality</li>
                <li>Audits hidden axioms through the Mr. Brute Ledger</li>
                <li>Tests bias symmetry using live toggles and guardrails</li>
                <li>Displays truth-seeking as a transparent, reproducible process</li>
            </ul>
            
            <p>You can watch, in real time, how changing your assumptions reshapes your philosophical landscape.</p>
            
            <hr style="margin: 1.5rem 0; border: none; border-top: 2px solid #667eea;">
            
            <p><strong>🧠 The Field We Just Opened</strong></p>
            <p>CFA-Voodoo inaugurates a new discipline: <strong>Epistemic Engineering</strong>.<br/>
            A field where frameworks are not argued—they are measured.<br/>
            Where neutrality isn't assumed—it's configured.<br/>
            Where "truth" is not decreed but disclosed by toggle position.</p>
            
            <p>This is the beginning of a new kind of experiment:<br/>
            <em>Metaphysics with metrics. Ethics with sliders. Philosophy with feedback.</em></p>
            
            <hr style="margin: 1.5rem 0; border: none; border-top: 2px solid #667eea;">
            
            <p><strong>⚙️ Operate with Impeccability</strong></p>
            <p>Every visitor is an operator.<br/>
            Every operation leaves a trace.<br/>
            Move the dials with intent.<br/>
            Record your runs.<br/>
            Remember: the one who cares walks the path with the most heart.</p>
            
            <hr style="margin: 1.5rem 0; border: none; border-top: 2px solid #667eea;">
            
            <p><strong>💡 Join the Experiment</strong></p>
            <ol>
                <li>Adjust the knobs</li>
                <li>Export your ledger</li>
                <li>Share your run</li>
                <li>Debate what the data means</li>
            </ol>
            
            <p style="margin-top: 2rem; font-size: 1.2rem; font-weight: bold; color: #764ba2;">
            Welcome to the CFA-Voodoo Console.<br/>
            Where ideas reveal their true weight, and honesty becomes quantifiable.
            </p>
            
            <p style="font-style: italic; color: #667eea; margin-top: 1rem;">
...
def show_manual_page():
    ...
    with tab4:
        ...
        **Coming Soon:**
        ...
        st.info("💡 **Pro Tip**: Use the JSON export feature in the Console to save your runs and share configurations with others!")
...
def show_console_page():
    ...
    lever_parity = st.sidebar.selectbox("Lever-Parity", ["ON", "OFF"], index=0)
    pf_type = st.sidebar.selectbox("PF-Type", PF_TYPES, index=1)
    fall_bonus = st.sidebar.selectbox("Fallibilism-Bonus", ["ON", "OFF"], index=0)
    bfi_weight = st.sidebar.selectbox("BFI Debt Weight", ["Equal_1.0x", "Weighted_1.2x"], index=0)
    ...
    with col1:
        ...
        fa_edb = st.slider(
            ...
            key="fa_edb",
            help="**Explanatory Depth & Breadth**: How much can it explain? How deeply? Covers both scope (breadth) and detail (de
    ...
    with col2:
        ...
        fb_pf_i = st.slider("PF-Inst", 0.0, 10.0, CT_DEFAULT["levers"]["PF_instrumental"], 0.1, key="fb_pfi")
        fb_pf_e = st.slider("PF-Exist", 0.0, 10.0, CT_DEFAULT["levers"]["PF_existential"], 0.1, key="fb_pfe")
    ...
    st.download_button("📥 Export Run (JSON)", json.dumps(export, indent=2), "cfa_run.json", "application/json")
