...
def render():
    ...
    # JSON Import/Export in Sidebar
    st.sidebar.markdown("**💾 Import/Export**")
    
    # Import Section
    with st.sidebar.expander("📥 Import JSON", expanded=False):
        ...
        if uploaded_file is not None:
            try:
                ...
                if "config" in run and "framework_a" in run and "framework_b" in run:
                    st.success("✅ Valid CFA profile")
                    
                    # Show preview
                    with st.expander("Preview"):
                        st.write(f"**A:** {run['framework_a'].get('name', 'Unknown')}")
                        st.write(f"**B:** {run['framework_b'].get('name', 'Unknown')}")
                    
                    if st.button("✅ Apply", use_container_width=True, key="sidebar_apply_json"):
    ...
    st.sidebar.markdown("---")
    st.sidebar.caption("💾 **Export** → See bottom of page")
    ...
    # ========================================================================
    # PREPARE EXPORT DATA (for use in sidebar and bottom)
    # ========================================================================
    export_data = {
        "config": cfg,
        "framework_a": fa,
    ...
    # ========================================================================
    # BOTTOM IMPORT/EXPORT SECTION
    # ========================================================================
    st.markdown("---")
    st.markdown("### 💾 Save or Load Configuration")
    ...
    with col_left:
        ...
        st.caption("*Save to reload later or share*")
    
    # Import
    with col_right:
        st.markdown("**📤 Import Saved Run**")
        uploaded_file_bottom = st.file_uploader(
            "Choose JSON file", 
            type=["json"], 
            key="json_upload_bottom",
            label_visibility="collapsed"
        )
        if uploaded_file_bottom is not None:
            try:
                run = json.load(uploaded_file_bottom)
                if "config" in run and "framework_a" in run and "framework_b" in run:
                    st.success("✅ Valid profile")
                    if st.button("Apply Configuration", key="bottom_apply_json"):
                        apply_loaded_run(run)
                        st.success("Loaded!")
                        st.rerun()
                else:
                ...
                st.error(f"Error: {e}")
        st.caption("*Load a previously saved audit*")
